...
from backend.models import db, ThreatLog
from backend.services.gmail_service import fetch_recent_emails
from backend.services.virustotal_service import check_urls_async
from backend.services.google_safebrowsing_service import check_urls_safebrowsing_async
from backend.services.alert_service import send_brevo_alert
from backend.utils.url_utils import extract_urls
from backend.utils.helpers import invalidate_dashboard_cache
...
async def scan_emails_async(limit: int = 5):
    ...
    try:
        if limit < 1 or limit > 50:
            logging.error(f"Invalid email limit: {limit}. Must be between 1 and 50")
            return []
        ...
        for batch_start in range(0, len(emails), BATCH_SIZE):
            ...
            for r in batch_results:
                if isinstance(r, Exception):
                    logging.error(f"Error processing email: {r}")
                else:
                    results.append(r)
...
def scan_emails(limit: int = 5):
    ...
    try:
        ...
        logging.error(f"Failed to scan emails: {e}", exc_info=True)
        return []
