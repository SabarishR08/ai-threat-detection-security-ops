...
from backend.services.threat_lookup_service import lookup_url, extract_domain
from backend.services.virustotal_service import check_url_virustotal
from backend.utils.constants import VT_STATUS_SAFE, VT_STATUS_MALICIOUS
...
class TestThreatLookup:
    ...
    def test_lookup_url_vt_error_graceful(self, mock_vt):
        ...
        assert "virustotal" in result
        # VT returns "Error" string on exception, not a dict with error key
        assert result["virustotal"] == "Error" or (isinstance(result["virustotal"], dict) and "error" in result["virustotal"])
