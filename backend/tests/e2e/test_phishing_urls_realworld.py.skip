...
def main():
    ...
    print(f"\n✅ Fetched {len(all_urls)} unique phishing URLs")
    ...
    for i, url in enumerate(all_urls, 1):
        ...
        print(f"\n[{i:2d}/{len(all_urls)}] {symbol} {url[:65]}")
        if result.get("success"):
    ...
    print("TEST SUMMARY")
    print(f"{'=' * 80}")
    print(f"✅ Malicious Detected: {malicious_count}/{len(all_urls)} ({100*malicious_count//len(all_urls) if all_urls else 0}%)")
    print(f"⚠️  Suspicious: {unknown_count}/{len(all_urls)}")
    print(f"🟢 Safe (False Negatives): {safe_count}/{len(all_urls)} ({100*safe_count//len(all_urls) if all_urls else 0}%)")
    print(f"❌ Errors: {error_count}/{len(all_urls)}")
